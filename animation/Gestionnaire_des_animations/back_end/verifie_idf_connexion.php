...
$req_pre = $cnx->prepare("SELECT mdp_hash FROM administration WHERE Emel_chiffre = :id");
$jour = date("N");
...
if ($ligne && password_verify($_POST['mot_de_passe'], $ligne->mdp_hash)) {
    ...
    if ($jour == 4) { //normalement c'est 2 pour le mardi mais pour l'exemple se sera vendredi
        $_SESSION['connect'] = true;
        $_SESSION['id'] = $identifiant_saisi;
        header('Location:../Gestionnaire_des_animations/front_end/verif_mardi.php');
        exit;
    }
    header('Location:../Gestionnaire_des_animations/front_end/traitement.php');
    exit;
} else {
    header('Location: ../Gestionnaire_des_animations/front_end/erreur.php?type=login');
    exit;
}
