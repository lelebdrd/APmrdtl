...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $phone = $_POST['phone'];
        $emel = $_POST['emel'];
    if (EMPTY($nom)=== true || EMPTY($prenom)=== true || EMPTY($phone)=== true || EMPTY($emel)=== true) {
        echo "une valeur est vide<br>";
    } else {
        $requete = $connexion->prepare("UPDATE inscrit SET nom = :nom, prenom = :prenom, tel = :tel, emel_chiffr = :emel_chiffr WHERE id = :id");
        ...
        $emel_chiffr = sodium_crypto_secretbox($emel, $nonce, $cle_secrete);
        $requete->bindParam(':id', $_GET['id']);
        $requete->bindParam(':emel_chiffr', $emel_chiffr);
        $requete->execute();
        header("Location: ../front_end/compte_eleves_admin.php");
    }
} else {
